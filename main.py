...
def get_product(id_group=None, search=None, limit=5, offset=0):
    if search:
        new_url = f"{urls['get_product']}?search={str(search)}&limit={limit}&offset={offset}"
        response = requests.get(url=new_url, headers=headers)
        get_products = response.json()
        return get_filter_products(get_products.get('rows'))
    if id_group:
        new_url = f"{urls['get_assortment']}{str(id_group)}&limit={limit}&offset={offset}"
        response = requests.get(new_url, headers=headers)
        get_products = response.json()
        return get_filter_products(get_products.get('rows'))
    new_url = f"{urls['get_product']}?limit={limit}&offset={offset}"
    response = requests.get(new_url, headers=headers)
    get_products = response.json()
    return get_filter_products(get_products.get('rows'))
